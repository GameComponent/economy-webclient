<template>
  <div>
    <search-player-storage v-if="!storageId || storageId === ''" @input="handleInputPlayerStorage" />
    <search-item v-if="!itemId || itemId === ''" @input="handleInputItem" />
  </div>
</template>
...
export default class GiveItem extends Vue {
  ...
  public handleInputPlayerStorage({ storageId, amount }) {
    ...
    this.$economyService.giveItem(this.request).then(() => {
